  ...
  if (!session?.user?.id) {
    return (
      <div className='flex min-h-screen flex-col items-center justify-center bg-white px-4 py-16 pt-20 text-center'>
        ...
        <p className='mt-2 text-sm text-slate-500'>Please sign in to change your password.</p>
        <Link href='/login?callbackUrl=/profile/change-password' className='mt-5 rounded-lg bg-orange-600 px-6 py-3 text-white hover:bg-orange-700'>
          Login to continue
        </Link>
      </div>
    );
